...
Synths are issuable by Synthetix Network Token (OKS) holders who
have to lock up some value of their OKS to issue S * Cmax synths.
...
contract Synth is ExternStateToken {
		...
		)
	{
		...
		require(
			ISynthetix(_synthetixProxy).synths(_currencyKey) == Synth(0),
			'Currency key is already in use'
		);
	...
	function issue(address account, uint256 amount) external onlySynthetixOrFeePool {
	...
	function burn(address account, uint256 amount) external onlySynthetixOrFeePool {
