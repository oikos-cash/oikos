...
contract Synthetix is ExternStateToken {
	 ...
	 * @param _synthetixState External immutable contract containing the OKS minters debt ledger.
	 * @param _owner The owner of this contract.
	 * @param _exchangeRates External immutable contract where the price oracle pushes prices onchain too.
	 * @param _feePool External upgradable contract handling OKS Fees and Rewards claiming
	 * @param _supplySchedule External immutable contract with the OKS inflationary supply schedule
	 * @param _rewardEscrow External immutable contract for OKS Rewards Escrow
	 * @param _escrow External immutable contract for OKS Token Sale Escrow
	 * @param _rewardsDistribution External immutable contract managing the Rewards Distribution of the OKS inflationary supply
	...
	function effectiveValue(
		...
		return
			exchangeRates.effectiveValue(sourceCurrencyKey, sourceAmount, destinationCurrencyKey);
	}
	...
	function totalIssuedSynths(bytes32 currencyKey) public view returns (uint256) {
		uint256 total = 0;
		uint256 currencyRate = exchangeRates.rateForCurrency(currencyKey);

		(uint256[] memory rates, bool anyRateStale) = exchangeRates.ratesAndStaleForCurrencies(
		...
		for (uint256 i = 0; i < availableSynths.length; i++) {
			...
			uint256 synthValue = availableSynths[i].totalSupply().multiplyDecimalRound(rates[i]);
		...
		returns (uint256)
	{
		...
		// Note: this assumes shorts begin with 'i' (0x69) and longs with 's' (0x73).
		if (
			(sourceCurrencyKey[0] == 0x73 &&
				sourceCurrencyKey != 'sUSD' &&
				destinationCurrencyKey[0] == 0x69) ||
			(sourceCurrencyKey[0] == 0x69 &&
				destinationCurrencyKey != 'sUSD' &&
				destinationCurrencyKey[0] == 0x73)
		) {
	 ...
	 */
	function transfer(address to, uint256 value) public optionalProxy returns (bool) {
		// Ensure they're not trying to exceed their staked OKS amount
		require(
			value <= transferableSynthetix(messageSender),
			'Cannot transfer staked or escrowed OKS'
		);
	...
	function transferFrom(
		...
		require(value <= transferableSynthetix(from), 'Cannot transfer staked or escrowed OKS');
    ...
    */
	function validateGasPrice(uint256 _givenGasPrice) public view {
	...
	function _internalExchange(
		...
		uint256 destinationAmount = effectiveValue(
			sourceCurrencyKey,
			sourceAmount,
			destinationCurrencyKey
		);
		...
		uint256 amountReceived = destinationAmount;
		uint256 fee = 0;

		if (chargeFee) {
			// Get the exchange fee rate
			uint256 exchangeFeeRate = feeRateForExchange(sourceCurrencyKey, destinationCurrencyKey);
		...
		if (fee > 0) {
			uint256 xdrFeeAmount = effectiveValue(destinationCurrencyKey, fee, 'ODR');
	 ...
	 */
	function _addToDebtRegister(bytes32 currencyKey, uint256 amount) internal {
		// What is the value of the requested debt in XDRs?
		uint256 xdrValue = effectiveValue(currencyKey, amount, 'ODR');

		// What is the value of all issued synths of the system (priced in XDRs)?
		uint256 totalDebtIssued = totalIssuedSynths('ODR');

		// What will the new total be including the new value?
		uint256 newTotalDebtIssued = xdrValue.add(totalDebtIssued);

		// What is their percentage (as a high precision int) of the total debt?
		uint256 debtPercentage = xdrValue.divideDecimalRoundPrecise(newTotalDebtIssued);
		...
		uint256 existingDebt = debtBalanceOf(messageSender, 'ODR');

		// And what does their debt ownership look like including this previous stake?
		if (existingDebt > 0) {
			debtPercentage = xdrValue.add(existingDebt).divideDecimalRoundPrecise(
				newTotalDebtIssued
			);
		}
	...
	/**
	 * @notice Issue synths against the sender's OKS.
	...
	/**
	 * @notice Issue the maximum amount of Synths possible against the sender's OKS.
	...
	function issueMaxSynths() external optionalProxy {
		...
		uint256 maxIssuable = remainingIssuableSynths(messageSender, currencyKey);
	...
	// No need to check for stale rates as effectiveValue checks rates
	{
		...
		uint256 debtToRemove = effectiveValue(currencyKey, amount, 'ODR');
		uint256 existingDebt = debtBalanceOf(messageSender, 'ODR');

		uint256 debtInCurrencyKey = debtBalanceOf(messageSender, currencyKey);

		require(existingDebt > 0, 'No debt to forgive');

		// If they're trying to burn more debt than they actually owe, rather than fail the transaction, let's just
		// clear their debt and leave them be.
		uint256 amountToRemove = existingDebt < debtToRemove ? existingDebt : debtToRemove;

		// Remove their debt from the ledger
		_removeFromDebtRegister(amountToRemove, existingDebt);

		uint256 amountToBurn = debtInCurrencyKey < amount ? debtInCurrencyKey : amount;
	 ...
	 */
	function _removeFromDebtRegister(uint256 amount, uint256 existingDebt) internal {
		...
		if (newTotalDebtIssued > 0) {
			...
			uint256 debtPercentage = debtToRemove.divideDecimalRoundPrecise(newTotalDebtIssued);
		...
		if (debtToRemove == existingDebt) {
			...
			uint256 newDebtPercentage = newDebt.divideDecimalRoundPrecise(newTotalDebtIssued);
		...
		)
	{
		...
		uint256 destinationValue = effectiveValue('OKS', collateral(issuer), currencyKey);
	...
	function collateralisationRatio(address issuer) public view returns (uint256) {
		uint256 totalOwnedSynthetix = collateral(issuer);
		if (totalOwnedSynthetix == 0) return 0;

		uint256 debtBalance = debtBalanceOf(issuer, 'OKS');
		...
		)
	{
		...
		uint256 currentDebtOwnership = synthetixState
			.lastDebtLedgerEntry()
	...
	{
		uint256 alreadyIssued = debtBalanceOf(issuer, currencyKey);
		uint256 max = maxIssuableSynths(issuer, currencyKey);
		...
		returns (uint256)
	{
		...
		uint256 lockedSynthetixValue = debtBalanceOf(account, 'OKS').divideDecimalRound(
	...
	function mint() external returns (bool) {
		...
		uint256 minterReward = supplySchedule.minterReward();
		// Get the remainder
		...
		tokenState.setBalanceOf(
			rewardsDistribution,
			tokenState.balanceOf(rewardsDistribution).add(amountToDistribute)
		);
