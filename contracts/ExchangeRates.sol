...
contract ExchangeRates is SelfDestructible {
	...
	// Exchange rates and update times stored by currency code, e.g. 'OKS', or 'sUSD'
	...
	uint256 public rateStalePeriod = 3 hours;

	// Each participating currency in the ODR basket is represented as a currency key with
	// equal weighting.
	// There are 5 participating currencies, so we'll declare that clearly.
	bytes32[5] public xdrParticipants;

	// A conveience mapping for checking if a rate is a ODR participant
		...
		SelfDestructible(_owner)
	{
		...
		// These are the currencies that make up the ODR basket.
		...
		// Mapping the ODR participants is cheaper than looping the xdrParticipants array to check if they exist
	...
	function getRateAndUpdatedTime(bytes32 code) internal view returns (RateAndUpdatedTime) {
		if (code == 'ODR') {
			// The ODR rate is the sum of the underlying ODR participant rates, and the latest
			// timestamp from those rates
			uint256 total = 0;
			uint256 lastUpdated = 0;
			for (uint256 i = 0; i < xdrParticipants.length; i++) {
			...
			return RateAndUpdatedTime({rate: uint216(total), time: uint40(lastUpdated)});
	 ...
	 * - lower of 100
	 *
	 * if this function is invoked with params iTRX and 184 (or rather 184e18),
	 * then the rate would be: 200 * 2 - 184 = 216. 100 < 216 < 200, so the rate would be 216,
	 * and remain unfrozen.
	 *
	 * If this function is then invoked with params iTRX and 301 (or rather 301e18),
	 ...
	 */
	function rateOrInverted(bytes32 currencyKey, uint256 rate) internal returns (uint256) {
		...
		uint256 newInverseRate = rates(currencyKey);
		...
		returns (uint256)
	{
		...
		return
			sourceAmount
				.multiplyDecimalRound(rateForCurrency(sourceCurrencyKey))
	 ...
	 */
	function rateForCurrency(bytes32 currencyKey) public view returns (uint256) {
	 ...
	 */
	function ratesForCurrencies(bytes32[] currencyKeys) public view returns (uint256[]) {
		...
		returns (uint256[], bool)
	{
		...
		for (uint256 i = 0; i < currencyKeys.length; i++) {
			...
			if (!anyRateStale) {
				anyRateStale = (currencyKeys[i] != 'sUSD' &&
					uint256(rateAndUpdateTime.time).add(period) < now);
			}
		}
	...
	function anyRateIsStale(bytes32[] currencyKeys) external view returns (bool) {
		...
		while (i < currencyKeys.length) {
			...
			if (
				currencyKeys[i] != 'sUSD' &&
				lastRateUpdateTimes(currencyKeys[i]).add(rateStalePeriod) < now
			) {
				return true;
			}
			i += 1;
