...
contract SynthetixState is State, LimitedSetup {
	...
	// Import state
	uint256 public importedXDRAmount;

	// A quantity of synths greater than this ratio
	// may not be issued against a given value of OKS.
	// x * 10 / 75 is same as x / 7.5
	uint256 public issuanceRatio = (SafeDecimalMath.unit() * 10) / 75;
	 ...
	 */
	function _addToDebtRegister(address account, uint256 amount) internal {
		...
		uint256 xdrValue = synthetix.effectiveValue('sUSD', amount, 'ODR');

		// What is the value that we've previously imported?
		uint256 totalDebtIssued = importedXDRAmount;
		...
		uint256 existingDebt = synthetix.debtBalanceOf(account, 'ODR');
