...
contract RewardEscrow is Owned {
	...
	/**
	 * @notice set the synthetix contract address as we need to transfer OKS when the user vests
	 */
	 ...
	 */
	function checkAccountSchedule(address account) public view returns (uint256[520]) {
	 ...
	 */
	function appendVestingEntry(address account, uint256 quantity) public onlyFeePool {
		...
		if (scheduleLength == 0) {
			...
			totalEscrowedAccountBalance[account] = totalEscrowedAccountBalance[account].add(
				quantity
			);
		}
	...
	function vest() external {
		...
		if (total != 0) {
			...
			totalEscrowedAccountBalance[msg.sender] = totalEscrowedAccountBalance[msg.sender].sub(
				total
			);
