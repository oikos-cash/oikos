...
contract Depot is SelfDestructible, Pausable {
	...
	mapping(uint256 => synthDeposit) public deposits;
	...
	constructor(
		...
		address _oracle,
		uint256 _usdToEthPrice,
		uint256 _usdToSnxPrice
	)
		public
	 ...
	 */
	function setPriceStalePeriod(uint256 _time) external onlyOwner {
	...
	function updatePrices(
		...
		require(
			timeSent < (now + ORACLE_FUTURE_LIMIT),
			'Time must be less than now + ORACLE_FUTURE_LIMIT'
		);
		...
		)
	{
		...
		uint256 requestedToPurchase = msg.value.multiplyDecimal(usdToEthPrice);
	...
	function exchangeEtherForSNX()
		...
		notPaused
		returns (
			uint256 // Returns the number of OKS received
		)
	{
	 ...
	 */
	function exchangeEtherForSynthetixAtRate(
	...
	{
		require(
			guaranteedEtherRate == usdToEthPrice,
			'Guaranteed ether rate would not be received'
		);
		require(
			guaranteedSynthetixRate == usdToSnxPrice,
	 ...
	 */
	function exchangeSynthsForSynthetix(uint256 synthAmount)
		public
		pricesNotStale
		notPaused
		returns (
	 ...
	 */
	function exchangeSynthsForSynthetixAtRate(uint256 synthAmount, uint256 guaranteedRate)
		public
		pricesNotStale
		notPaused
		returns (
	...
	function synthetixReceivedForSynths(uint256 amount) public view returns (uint256) {
		...
		uint256 synthsReceived = feePool.amountReceivedFromTransfer(amount);
	...
	function synthetixReceivedForEther(uint256 amount) public view returns (uint256) {
		...
		uint256 valueSentInSynths = amount.multiplyDecimal(usdToEthPrice);
	...
	function synthsReceivedForEther(uint256 amount) public view returns (uint256) {
		...
		uint256 synthsTransferred = amount.multiplyDecimal(usdToEthPrice);
	...
	event PriceStalePeriodUpdated(uint256 priceStalePeriod);
	event PricesUpdated(uint256 newEthPrice, uint256 newSynthetixPrice, uint256 timeSent);
