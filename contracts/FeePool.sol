...
contract FeePool is Proxyable, SelfDestructible, LimitedSetup {
	...
	uint256 public constant MAX_EXCHANGE_FEE_RATE = SafeDecimalMath.unit() / 10;
	...
	uint256 public targetThreshold = (10 * SafeDecimalMath.unit()) / 100;
	 ...
	 * @dev onlySynthetix to call me on synthetix.issue() & synthetix.burn() calls to store the locked OKS
	 ...
	 */
	function setExchangeFeeRate(uint256 _exchangeFeeRate) external optionalProxy_onlyOwner {
	 ...
	 */
	function setFeePeriodDuration(uint256 _feePeriodDuration) external optionalProxy_onlyOwner {
	...
	function setTargetThreshold(uint256 _percent) external optionalProxy_onlyOwner {
	 ...
	 */
	function recordFeePaid(uint256 xdrAmount) external onlySynthetix {
		// Keep track of in ODRs in our fee pool.
		_recentFeePeriodsStorage(0).feesToDistribute = _recentFeePeriodsStorage(0)
			.feesToDistribute
			.add(xdrAmount);
	}
	...
	function setRewardsToDistribute(uint256 amount) external {
		...
		_recentFeePeriodsStorage(0).rewardsToDistribute = _recentFeePeriodsStorage(0)
			.rewardsToDistribute
			.add(amount);
	}
	...
	function claimOnBehalf(address claimingForAddress) external optionalProxy returns (bool) {
		require(
			delegates.approval(claimingForAddress, messageSender),
	 ...
	 */
	function appendVestingEntry(address account, uint256 quantity) public optionalProxy_onlyOwner {
	 ...
	 */
	function _recordFeePayment(uint256 xdrAmount) internal returns (uint256) {
		// Don't assign to the parameter
		uint256 remainingToAllocate = xdrAmount;
	...
	function _recordRewardPayment(uint256 snxAmount) internal returns (uint256) {
		...
		for (uint256 i = FEE_PERIOD_LENGTH - 1; i < FEE_PERIOD_LENGTH; i--) {
			...
			if (toDistribute > 0) {
				...
				_recentFeePeriodsStorage(i).rewardsClaimed = _recentFeePeriodsStorage(i)
					.rewardsClaimed
					.add(amountInPeriod);
				remainingToAllocate = remainingToAllocate.sub(amountInPeriod);
	...
	function _payFees(
		...
		uint256 destinationAmount = synthetix.effectiveValue(
			'ODR',
			xdrAmount,
	 ...
	 */
	function amountReceivedFromTransfer(uint256 value) external pure returns (uint256) {
		return value;
	}
	 ...
	 */
	function exchangeFeeIncurred(uint256 value) public view returns (uint256) {
	 ...
	 */
	function totalFeesAvailable(bytes32 currencyKey) external view returns (uint256) {
		uint256 totalFees = 0;
		...
		returns (uint256, uint256)
	{
		...
		return (synthetix.effectiveValue('ODR', totalFees, currencyKey), totalRewards);
	}
	...
	function isFeesClaimable(address account) public view returns (bool) {
		...
		uint256 ratio = synthetix.collateralisationRatio(account);
		uint256 targetRatio = synthetixState.issuanceRatio();
		...
		returns (uint256[2][FEE_PERIOD_LENGTH] memory results)
	{
		...
		for (uint256 i = FEE_PERIOD_LENGTH - 1; i > 0; i--) {
			uint256 next = i - 1;
			uint256 nextPeriodStartingDebtIndex = _recentFeePeriodsStorage(next).startingDebtIndex;
			...
			if (
				...
				uint256 closingDebtIndex = uint256(nextPeriodStartingDebtIndex).sub(1);
	...
	function _feesAndRewardsFromPeriod(
		...
		uint256 feesFromPeriod = _recentFeePeriodsStorage(period).feesToDistribute.multiplyDecimal(
	...
	function _effectiveDebtRatioForPeriod(
		...
		// This is a high precision integer.
		uint256 feePeriodDebtOwnership = synthetixState
		...
		returns (uint256)
	{
		...
		uint256 closingDebtIndex = uint256(_recentFeePeriodsStorage(period - 1).startingDebtIndex)
			.sub(1);

		uint256 ownershipPercentage;
		uint256 debtEntryIndex;
		(ownershipPercentage, debtEntryIndex) = feePoolState.applicableIssuanceData(
			account,
			closingDebtIndex
		);
	...
	event ExchangeFeeUpdated(uint256 newFeeRate);
	bytes32 constant EXCHANGEFEEUPDATED_SIG = keccak256('ExchangeFeeUpdated(uint256)');

	function emitExchangeFeeUpdated(uint256 newFeeRate) internal {
	...
	function emitFeePeriodDurationUpdated(uint256 newFeePeriodDuration) internal {
	...
	event FeesClaimed(address account, uint256 xdrAmount, uint256 snxRewards);
